...
def send_email(title,msg,recipient):
  #
  msg = "Sending email to " + recipient + ". Subj: " + title + ". " + msg
  logmsg("DEBUG",msg)
  anvil.email.send(
    to=recipient,
...
def execute_sql_command(sql_cmd):
  ...
  if sql_cmd[0:6] == "SELECT":
    ...
    msg = "FAIL: SQL command failed (not started with SELECT): " + sql_command
    logmsg("ERROR",msg)
  return msg, pd_list, col_order
...
client_info["highlight_colour"] = config.get("client","highlight_colour")
...
logmsg('INFO',msg)
#send_email("Anvil app server tartup","Startup completed","tony.bakker@berksarch.co.uk")
